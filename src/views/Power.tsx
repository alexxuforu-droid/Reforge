...
export default function Power() {
  ...
  return (
    <div className="space-y-4">
            ...
            {health && (
              <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                <div className="rounded-lg border border-[var(--border-default)] bg-[var(--surface-overlay)] p-2">
                  ...
                  <div className="text-2xs text-[var(--text-tertiary)]">{t("power.health")}</div>
                </div>
                <div className="rounded-lg border border-[var(--border-default)] bg-[var(--surface-overlay)] p-2">
                  <div className="text-lg font-bold text-[var(--text-primary)]">{health.cycle_count != null ? health.cycle_count : "—"}</div>
                  <div className="text-2xs text-[var(--text-tertiary)]">{t("power.cycles")}</div>
                </div>
                <div className="rounded-lg border border-[var(--border-default)] bg-[var(--surface-overlay)] p-2">
                  <div className="text-lg font-bold text-[var(--text-primary)]">{health.full_mwh != null ? Math.round(health.full_mwh / 1000) : "—"} Wh</div>
                  <div className="text-2xs text-[var(--text-tertiary)]">{t("power.fullCapacity")}</div>
                </div>
              </div>
            )}
          </Section>

          <Section title={t("power.plan")} subtitle={t("power.plan.subtitle")}>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Section title={t("power.screenOff")} subtitle={t("power.screenOff.subtitle")}>
          <div className="flex items-end gap-3">
            ...
            <button className="btn-primary" disabled={busy} onClick={() => setScreenOff(acMin ?? data?.screen_off_ac_min ?? 10, dcMin ?? data?.screen_off_dc_min ?? 5)}>
              <IconMonitor size={13} /> {t("power.apply")}
            </button>
          </div>
        </Section>

        <Section title={t("power.hibernate")} subtitle={t("power.hibernate.subtitle")}>
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm font-medium text-[var(--text-primary)]">{t("power.hibernate")}</div>
              <div className="text-2xs text-[var(--text-tertiary)]">
                {data?.hibernate_supported === false ? t("power.hibernate.unavailable") : t("power.hibernate.desc")}
              </div>
            </div>
            <Toggle on={data?.hibernate_enabled ?? false} disabled={!data?.hibernate_supported || busy} onChange={toggleHibernate} label={t("power.hibernate")} />
          </div>
          <div className="mt-3 flex items-center gap-1.5 text-2xs text-[var(--text-tertiary)]">
            <IconPower size={11} /> {t("power.hibernate.admin")}
          </div>
        </Section>
      </div>
      ...
      {!battery && !error && (
        <div className="flex items-center gap-2 rounded-lg border border-[var(--border-default)] bg-[var(--surface-overlay)] px-3 py-2 text-xs text-[var(--text-secondary)]">
          <IconGauge size={14} /> {t("power.noBattery")}
        </div>
      )}
    </div>
  );
